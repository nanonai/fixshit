...
public class ViewSuppliers {
    ...
    public static void ShowPage() {
        ...
        sup_list = new Supplier().ListAll();
        Object[][] data = new Object[sup_list.size()][titles.length];
        int counter = 0;
        for (Supplier supplier : sup_list) {
            data[counter] = new Object[]{supplier.getSupplierID(), supplier.getSupplierName(), supplier.getContactPerson(),
        ...
        add.addActionListener(_ -> {
            if (table_sup.getSelectedRowCount() == 0) {
                ...
                SupplierDetails.UpdateSupplier(new Supplier().GetObjectByID(selected_id));
    ...
    public static void SearchStuff() {
        String searcher = (!search.getText().isEmpty() && !Objects.equals(search.getText(), "Search...\r\r")) ?
                search.getText() : "";
        List<Supplier> temp_user_list = new Supplier().ListAllWithFilter(searcher);
