...
public class ViewItems {
    ...
    private static CustomComponents.CustomButton add;
    ...
    public static void ShowPage() {
        ...
        String[] titles = new String[]{"Id", "Name", "Unit Price", "Unit Cost", "Stock Count", "Threshold", "Category"};
        item_list = new Item().ListAll();
        Object[][] data = new Object[item_list.size()][titles.length];
        int counter = 0;
        for (Item item : item_list) {
            data[counter] = new Object[]{item.getItemID(), item.getItemName(), item.getUnitPrice(),
                    item.getUnitCost(), item.getStockCount(), item.getThreshold(), item.getCategory()};
            counter += 1;
        }
        ...
        add.addActionListener(_ -> {
            if (table_item.getSelectedRowCount() == 0) {
                ...
                ItemDetails.UpdateItem(new Item().GetObjectByID(selected_id));
                boolean see = ItemDetails.ShowPage();
                if (see) {
                    System.out.println(" ");
                }
            }
    ...
    public static void SearchStuff() {
        String searcher = (!search.getText().isEmpty() && !Objects.equals(search.getText(), "Search...\r\r")) ?
                search.getText() : "";
        List<Item> temp_user_list = new Item().ListAllWithFilter(searcher);
